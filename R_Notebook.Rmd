...
# Biomart
We used Biomart for to generate ENSEMBL gene IDS format (required for gene functional annotation on DAVID web server).
...
```{r}
tag.snps1 <- read.table('data/SOIs/snps_of_interest_top10.txt')
snps1 <- unique(sort(as.vector(unlist(tag.snps))))
snp1 <- snps1[1]
...
for (snp1 in snps1) {
  #ext <- "/ld/human/rs1042779/1000GENOMES:phase_3:KHV?content-type=application/json"
  ext <- paste(sep="", "/variation/human/", snp1, "?content-type=application/json")
...
```{bash}
cat ~/Dropbox/rSNPs/data/SOIs/snps_of_interest.txt |python -c "import sys;sys.stdout.write('select chrom,chromStart,chromEnd,name from snp150 where name in (\'');[sys.stdout.write('%s'%line.replace('\n', '\',\'')) for line in sys.stdin];sys.stdout.write('\')')" |mysql --user=genome --host=genome-mysql.cse.ucsc.edu -NA -D hg38 |sort -k1,1 -k2,2n > ~/Dropbox/rSNPs/results/SOIs.bed
```
...
## wget 1000 genomes phase 3 

export GENOME1K_DIR_URL=ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502
export GENOME1K_PUBLIC_DIR=/Users/yvonfrid/human_genome/${GENOME1K_DIR_URL}
...
We keep African (AFR) population.

```{bash plink_bed}
export GENOME1K_DIR_URL=ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502
export GENOME1K_PUBLIC_DIR=/Users/yvonfrid/human_genome/${GENOME1K_DIR_URL}
export RESULT_DIR=/Users/yvonfrid/Dropbox/rSNPs/results
export RES_DIR_AFR=${RESULT_DIR}/AFR
mkdir -p ${GENOME1K_PUBLIC_DIR}
mkdir -p ${RESULT_DIR}
mkdir -p ${RES_DIR_AFR}

grep AFR ${GENOME1K_PUBLIC_DIR}/integrated_call_samples_v3.20130502.ALL.panel | cut -f1 > $RES_DIR_AFR/AFR.id
head $RES_DIR_AFR/AFR.id
```


### Convert to plink BED 

```{bash convert_plink_to_bed}
export GENOME1K_DIR_URL=ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502
export GENOME1K_PUBLIC_DIR=/Users/yvonfrid/human_genome/${GENOME1K_DIR_URL}
export RESULT_DIR=/Users/yvonfrid/Dropbox/rSNPs/results
export RES_DIR_AFR=${RESULT_DIR}/AFR
export RES_DIR_PLINK=${RESULT_DIR}/PLINK_BED
export SNP_IDS=/Users/yvonfrid/Dropbox/rSNPs/data/SOIs
mkdir -p ${GENOME1K_PUBLIC_DIR}
mkdir -p ${RESULT_DIR}
mkdir -p ${RES_DIR_AFR}
mkdir -p ${RES_DIR_PLINK}
mkdir -p ${SNP_IDS}

#seq 1 22 | xargs -P 6 -I {} sh -c "plink --vcf ${GENOME1K_PUBLIC_DIR}/ALL.chr{}.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz --keep-fam $RES_DIR_AFR/AFR.id --geno 0.05 --hwe 1e-6 --make-bed --out $RES_DIR_PLINK/chr{}"

for CHR in `seq 1 22`; do 
echo plink --vcf ${GENOME1K_PUBLIC_DIR}/ALL.chr${CHR}.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz --keep-fam $RES_DIR_AFR/AFR.id --geno 0.05 --hwe 1e-6 --make-bed --out  $WDIR/$RES_DIR_PLINK/chr${CHR}
done

WDIR=${GENOME1K_PUBLIC_DIR}/tmp/plink_show-tags
rm -f ${WDIR}/plink.tags && rm -rf ${WDIR}/chrom && mkdir -p ${WDIR}/chrom
cd ${WDIR}
touch ${WDIR}/snps_of_interest.txt
```
...
```{r}
C0024530.data <- read.delim('~/Google\ Drive/These_Yvon_2018/malaria_rSNPs/C0024530_Malaria.tsv', header = TRUE, sep = "\t")

C0024534.data <- read.delim('~/Google\ Drive/These_Yvon_2018/malaria_rSNPs/C0024534_Cerebral_malaria.tsv', header = TRUE, sep = "\t")

C0747820.data <- read.delim('~/Google\ Drive/These_Yvon_2018/malaria_rSNPs/C0747820_Antepartum_malaria.tsv', header = TRUE, sep = "\t")

C1836721.data <- read.delim('~/Google\ Drive/These_Yvon_2018/malaria_rSNPs/C1836721_Mild_malaria_susceptibility.tsv', header = TRUE, sep = "\t")

C1855457.data <- read.delim('~/Google\ Drive/These_Yvon_2018/malaria_rSNPs/C1855457_Cerebral_malaria_susceptibility.tsv', header = TRUE, sep = "\t")

C1969379.data <- read.delim('~/Google\ Drive/These_Yvon_2018/malaria_rSNPs/C1969379_Cerebral_malaria_resistance.tsv', header = TRUE, sep = "\t")

C1970028.data <- read.delim('~/Google\ Drive/These_Yvon_2018/malaria_rSNPs/C1970028_Malaria_susceptibility.tsv', header = TRUE, sep = "\t")

C0024530.data <- read.delim('~/Google\ Drive/These_Yvon_2018/malaria_rSNPs/C0024530_Malaria.tsv', header = TRUE, sep = "\t")

gene.malaria.disgenet <- rbind(C0024530.data, C0024534.data, C0747820.data, C1836721.data, C1855457.data, C1969379.data, C1970028.data, C0024530.data)

## Export the full result table

data.file <- "gene.malaria.disgenet.tsv"
data.path <- file.path(dir.path["disgenet"], data.file)
write.table(x = gene.malaria.disgenet,
            file = paste(dir, "disgenet", data.file, sep = "/"), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
