...
```{bash}
cat ~/Google\ Drive/These_Yvon_2018/malaria_rSNPs/results/sois/snps_of_interest.txt |python -c "import sys;sys.stdout.write('select chrom,chromStart,chromEnd,name from snp150 where name in (\'');[sys.stdout.write('%s'%line.replace('\n', '\',\'')) for line in sys.stdin];sys.stdout.write('\')')" |mysql --user=genome --host=genome-mysql.cse.ucsc.edu -NA -D hg38 |sort -k1,1 -k2,2n > ~/Google\ Drive/These_Yvon_2018/malaria_rSNPs/results/sois/sois.bed
```
