...
gene_snp_disease <- rbind(gwas_2014_vs_2018, signif_catalog_gwas)
...
kable(head(TagSnps), caption = "Liste of TagSNPS")
...
**Tag SNPs result file of  malaria: [`r tagsnp.file`](`r tagsnp.path`)**
...
snps <- unique(sort(as.vector(unlist(TagSnps))))
snp <- snps[1]
...
for (snp in snps) {
  ...
  ldResult <- (fromJSON(toJSON(content(r))))
  #View(head(ld.Result))
 return(ldResult)
}
```
...
## Analysis of SNPs of interest

```{r}
sois.gwava <- read.delim('data/gwava/sois_vs_gwava.tsv', header = TRUE, sep = "\t")
...
## Select a subset of fields for display in the report
  ...
  ## Display a small subset of the table
  gwava_table <- sois.gwava[, gwava.fields]
