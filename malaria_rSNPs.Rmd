...
## IDs of the diseases of interesd according to the SNPs
diseaseSnpIDs <- c("Malaria snp" = "C0024530", 
                "P.falciparum Malaria snp" = "C0024535",
                "P.vivax Malaria snp" = "C0024537",
                "Malaria snp susceptibility" = "C1970028",
                "Antepartum malaria snp" = "C0747820",
                "Mild malaria snp susceptibility" = "C1836721",
                "Cerebral malaria snp" = "C0024534",
                "Cerebral malaria snp susceptibility" = "C1855457",
                "Cerebral malaria snp resistance" = "C1969379")

## Summarize the genes per disease from disgenet
snp.summary <- data.frame(
  row.names = diseaseSnpIDs,
  ID = diseaseSnpIDs,
  name = names(diseaseSnpIDs))
kable(snp.summary)
...
for (s in 1:length(diseaseSnpIDs)) {
  ...
  snp.data <- GetSnpsForDisease(diseaseSnpIDs, diseaseName)
  ...
  snp.data.path1 <- file.path(dir.path["snp.disgenet"], snp.data.file1)
...
```{r}
print(ggplot(snp.data.all.diseases, aes(c2.most_severe_consequence	)) + geom_bar() + coord_flip())
print(ggplot(snp.data.all.diseases, aes(c1.name)) + geom_bar() + coord_flip())
```
...
all_snps_disgenet <- read_tsv('results/snp.disgenet/all_snps_disease_association.tsv')
snp2populations <-
  all_snps_disgenet %>%
  filter(c0.Npmids > 1)
...
Teh Tag SNPs are the result of the combinaison of the SNPs from DisGeNet and GWAS (2014, 2018)
...
tagSnpsLdHaploreg <- queryHaploreg(query = TagSnp, ldThresh = 0.8, ldPop = "AFR", url = "http://archive.broadinstitute.org/mammals/haploreg/haploreg.php")
...
```{r rsid_vs_ld_snps}
 LdTagsnps <- unique(subsetLd$rsID, subsetLd$query_snp_rsid)
#LdTagsnps <- as.character(LdTagsnps)
LdTagsnps <- as.data.frame(LdTagsnps)
#names(LdTagsnps) <- "rsid"
LdTagsnps <- unique(LdTagsnps)
